Overview
This is a collection of algorithms I wrote to solve “Number Mind” puzzles, as introduced in Project Euler problem 185. I wrote these because this problem seemed to have a particularly diverse set of available approaches, and I thought it would be fun to try to learn new ones and optimize all of them as much as I could. You should probably read the problem description first, and of course if you want to solve the problem yourself, you should do that before continuing.* Thanks very much to Chris Gearheart for helping with these algorithms and telling me about MIMIC.
...
Cost function
Many of the above algorithms used a cost function. For a given state, my cost function was the sum of how “wrong” the guess scores were, when the guesses were compared to state in question, with the wrongness being the absolute value of the difference in how many were correct vs how many were supposed to be correct. One interesting implementation issue that came up was that the optimal way to calculate cost varied by algorithm. For the genetic algorithm, costs were basically calculated from scratch each time, but for something like hill-descent or simulated annealing, where only one digit was changing, it was faster to store some additional information 
about the breakdown of the cost and use that to get the next cost.

Bayesian probabilities
Since the guesses allow for only one possible answer, this isn’t a probability question, but if the given information is reduced to occurrences and non-occurrences of a particular digit in a particular place, it’s then possible to calculate the probability of that digit/place combination. The underlying assumption is that the guesses were picked randomly when the puzzle was generated, which is very unlikely, given the unnatural distribution of scores for the guesses (only one 0, and the rest between 1 and 3). However, I *think* the formula I derived applies to any method of puzzle creation in which the guesses are generated randomly, even if guesses are sometimes rejected based on their scores. And the results seem to endorse these probability distributions, since many algorithms perform substantially better when using Bayesian probabilities.
...
*Project Euler strongly discourages participants from posting solutions online, but I felt this post was in keeping with the spirit of learning/exploration rather than spoiling. Plus the problem is ten years old.